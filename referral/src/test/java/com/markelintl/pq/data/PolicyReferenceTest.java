...
public class PolicyReferenceTest {
    ...
    public void toString_should_concatenate_all_fields_in_ascending_order_by_field_name() throws IOException, ParseException {
        ...
        assertThat(policy.toString(), is("TorontoCanadaCUST1234EST2017-01-012016-01-01test@mklquote.comNate Fisher200 Wellington Street WestSuite 400PR1234BR1234M5V 3C7Ontario"));
    }

    @Test
    public void parseDate_should_apply_timezone_correctly() throws ParseException {
        final Object[][] testData = {
                {"2016-01-01",   "GMT", new LocalDate(1451606400000L, DateTimeZone.forID("GMT"))},
                {"2016-01-01",   "",    new LocalDate(1451606400000L, DateTimeZone.forID("GMT"))},
                {"2016-01-01",   "EST", new LocalDate(1451624400000L, DateTimeZone.forID("EST"))},
                {1451624400000L, "EST", new LocalDate(1451624400000L, DateTimeZone.forID("EST"))}
        };

        int i = 0;
        for (final Object[] td: testData) {
            final String tz = (String) td[1];
            assertThat("parseDate(testData[" + i + "]{" + td[0] + "}, " + tz + ")", parseDate(td[0], tz), is(equalTo(td[2])));
            i++;
        }
    }
}